...
{
    public class LogActionFiltersAttribute : ActionFilterAttribute
    {
        ...
        public LogActionFiltersAttribute(IWebHostEnvironment environment)
        {
            _environment = environment;
            _contentRootPath = environment.ContentRootPath;
            _logPath = _contentRootPath + "\\LogFile\\";
            _fileName = $"log {DateTime.Now.ToString("MM-dd-yyyy-H-mn")}.txt";
        ...
        public override void OnResultExecuted(ResultExecutedContext filterContext)
        {
            ...
            using (FileStream fs = new FileStream(_fullPath, FileMode.Append))
            {
                using (StreamWriter sw = new StreamWriter(fs))
                {
                    sw.WriteLine($"The action {actionName} in {controllerName} controller has the followinf viewData: {result.ViewData.Values.FirstOrDefault()}, event fired: OnResultExecuted");
                }
            }
        }
    }
